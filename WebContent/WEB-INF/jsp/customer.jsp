...
<body>
				...
				<div class="col-lg-12">
					<div class="panel panel-default">
							...
							<tbody>
								<c:forEach items="${page.rows}" var="row">
									<tr>
										...
										<td>${row.cust_name}</td>
										<c:forEach items="${fromType}" var="item">
											<c:if test="${item.dict_id==row.cust_source }"><td>${item.dict_item_name }</td></c:if>
											<%-- <td>${row.cust_source}</td> --%>
										</c:forEach>
										
										<%-- <td>${row.cust_industry}</td> --%>
										<c:forEach items="${industryType}" var="item">
											<c:if test="${item.dict_id==row.cust_industry }"><td>${item.dict_item_name }</td></c:if>
										</c:forEach>
										<%-- <td>${row.cust_level}</td> --%>
										<c:forEach items="${levelType}" var="item">
											<c:if test="${item.dict_id==row.cust_level }"><td>${item.dict_item_name }</td></c:if>
										</c:forEach>
						...
						<div class="col-md-12 text-right">
							<ccc:page url="${pageContext.request.contextPath }/customer/list.action" />
						</div>
						<!-- /.panel-body -->
					</div>
					<!-- /.panel -->
	...
	<script type="text/javascript">
		...
		function createCustomer() {
			$.post("<%=basePath%>customer/create.action",$("#add_customer_form").serialize(),function(data){
				alert("新增客户成功！");
				window.location.reload();
			});
		}
